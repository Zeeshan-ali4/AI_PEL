"""Shared fixtures for T23 policy rule editor tests.

Mirrors the real-OPA pattern used by tests/T15_scenarios_ui through
tests/T19_settings_ui: a real `opa` binary is started against the real
policy/data bundle so these acceptance tests exercise the real settings
store + pipeline + OPA path, never a hand-written or post-filtered decision.
"""
...
def opa_url() -> Iterator[str]:
    """Provide a real OPA/Rego HTTP endpoint for the rule-editor acceptance tests."""
