"""Shared fixtures for T22 live event feed tests.

Mirrors the real-OPA pattern used by tests/T18_audit_ui/conftest.py: a real
`opa` binary is started against the real policy/data bundle so these
acceptance tests exercise the real pipeline, not a mocked policy engine.
"""
...
def opa_url() -> Iterator[str]:
    """Provide a real OPA/Rego HTTP endpoint for the T22 acceptance tests."""
...
def wired_pipeline(tmp_path, monkeypatch, opa_url) -> pipeline_module.PolicyPipeline:
    ...
    and make the SSE stream's background-event pacing instant for fast tests."""
    ...
    monkeypatch.setattr(routes_module, "EVENT_STREAM_DELAY_SECONDS", 0)
    return pipe
